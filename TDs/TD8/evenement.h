...
// devra etre definie dans les classes filles pour les rendre instanciables 
		...
		void afficher(std::ostream& f=std::cout) const { f<<toString(); } // EXERCICE 31 - QUESTION 3
		const std::string& getDescription() const { return sujet; }
		virtual ~Evt(){}
// FACTORY METHOD
		virtual Evt * clone() const =0; // delegue la responsabilite de fournir une methode de duplication aux classes filles EXERCICE 31 - QUESTION 2
// quand on a un objet dont on ne connait pas le type ?
// TEMPLATE METHOD
		virtual std::string toString() const =0; // delegue la responsabilite de fournir une methode de description aux classes filles
	};
...
// EvtPj "est un" Evt avec date de debut et date de fin
	...
	public:
		...
		// }	
		EvtPj * clone() const; 
// Le type retour dans une classe derivee peut etre different du type de base sous les contraintes : 
// 	- la methode doit renvoyer un pointeur ou une reference
// 	- le type retour de la classe derivee doit deriver du type retour de la classe de base
// 	On parle de type "covariant".
...
// accesseurs pour les nouveaux attributs
		...
		std::string toString() const;
		bool operator<(const Evt1j & e) const;
	};
...
// 10 evenements geres par defaut dans l'agenda
		...
		~Agenda();
		Agenda & operator<<(const Evt &e); // operateur d'insertion d'un evenement dans l'agenda
		void afficher(std::ostream& f=std::cout) const;
